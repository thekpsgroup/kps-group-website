...
async function sendEmailViaSMTP(data: EmailData) {
	...
	try {
		const response = await fetch('https://api.mailgun.net/v3/thekpsgroup.com/messages', {
			method: 'POST',
			headers: {
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		if (emailSent) {
			...
			return json({ 
				success: true, 
				message: 'Lead captured and logged. Email delivery will be attempted via backup methods.',
